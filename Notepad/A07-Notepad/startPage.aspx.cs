...
namespace A07_Notepad
{
    public partial class startPage : System.Web.UI.Page
    {
        ...
        public static string UsrPrintFeedback(string message)
        {
            ...
            if (fileStatus != null)
            {
                string[] splitted = fileStatus.Split(new string[] { "|" }, 2, StringSplitOptions.None);    // add | in file names
                fileStatus = splitted[0];
                if (splitted.Count() > 1)
                {
                    fileContents = splitted[1];
                }
            }
        ...
        public static string UsrFileSaveAs(string message)
        {
            ...
            if (result == false)
            {
                fileStatus = "Failure - File Must Have an Extension";
                fileContents = text;
